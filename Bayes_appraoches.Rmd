...
post.dat <- rgamma(100, shape = 23, rate = 1)
...
model {
  ...
  # Likelihood
  for (i in 1:N) {
    mean[i] <- ifelse(time[i] == time[1], mu_pre, mu_post)
    density[i] ~ dgamma(shape, 1 / mean[i])
  }
...
```{r}
## Specify ##
mod <- jags.model(textConnection(jags_code), data = list(N = nrow(dat.sim), density = dat.sim$density, time = as.integer(factor(dat.sim$time))), n.chains = 3)

## Burn ##
update(mod, 1000)
samples <- coda.samples(mod, variable.names = c("shape", "mu_pre", "mu_post", "delta"), n.iter = 5000)
