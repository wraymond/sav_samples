...
model {
  ## Likelihood ##
  for (t in 1:N) { # Loop through observations
    mu[t] <- Beta[time[t]] # Use 'time' as an index to get the group mean
    y[t] ~ dpois(mu[t]) # Values treated as Poisson-distributed
  }
  ...
  ## Prior for Poisson parameter (mean) ##
  lambda ~ dgamma(0.001, 0.001)
}
"

## Initial values ##
initial_values <- list(Beta = rep(0, d$p))

## Fit ##
mod.bys <- jags.model(textConnection(jags.mod), data = d, inits = initial_values)
```
...
summary(lm(density~time, data = dat.sim))
```
