...
### 1 
Simulate 50 data points form a Poisson with mean 6.4. We are calling this our example data. 
```{r}
set.seed(10)
...
```{r}
plot(theta, like(theta, y = y), type = "l", xlim = c(0, 15), main = "Likelihood", xlab = expression(theta), 
...
plot(theta, joint(theta), type = "l",  main = "Joint", xlim = c(0, 15), xlab = expression(theta),
...
plot(theta, p.theta, typ = "l", xlim = c(0, 15), main = "Posterior", xlab = expression(theta), 
...
plot(theta, prior(theta), type = "l", ylab = expression(paste("[", theta, "]")), xlab = expression(theta),
  main = "Prior", xlim = c(0, 15))

hist(y, freq = FALSE, breaks = 10, main = "Histogram of data")
discrete_hist(y = y)

plot(theta, like(theta, y = y), type = "l", main = "Likelihood", xlim = c(0, 15), xlab = expression(theta),
  ylab = expression(paste("[y|", theta, "])")))
plot(theta, joint(theta), type = "l", main = "Joint", xlim = c(0, 15), xlab = expression(theta),
  ylab = expression(paste("[y | ", theta, "]) x [", theta, "]")))
plot(theta, p.theta, type = "l", xlim = c(0, 15), main = "Posterior", xlab = expression(theta),
  ylab = expression(paste("[ ", theta, " | y]")))
```
